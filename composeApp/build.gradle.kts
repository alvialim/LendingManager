...
compose.desktop {
    application {
        ...
        /**
         * Desktop Firebase (GitLive JVM / gRPC / Netty) can fail cloud writes on module-strong JDKs with errors
         * mentioning `sun/misc/Unsafe` unless a few `java.base` packages are opened for unnamed modules.
         *
         * These flags apply to `run` and packaged installers (msi/exe/dmg/deb) generated by Compose Desktop.
         *
         * Note: Avoid JDK-version-specific flags here (e.g. JDK 23+ only) — they can prevent startup on JDK 17/21.
         */
        jvmArgs += listOf(
            // Prefer a Netty path that avoids sun.misc.Unsafe when possible (helps low-spec Windows machines too).
            "-Dio.netty.noUnsafe=true",
            // Typical gRPC/Netty requirements on module-strong JDKs (17+).
